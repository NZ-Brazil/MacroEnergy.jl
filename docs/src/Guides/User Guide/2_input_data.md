...
| **max_supply** | `Vector` or `Dict` | Vector of numbers, vector of vectors, or timeseries spec | [0.0] | Maximum allowed supply per segment. **Legacy**: `[1000.0, 2000.0]` — one value per segment, constant over time. **Time-varying**: `[[100,101,...], [200,201,...], ...]` — each inner array is the time series for that segment. **From file**: `{"timeseries": {"path": "system/max_supply.csv", "header": "bioherb_MA", "segments": 3}}` — CSV columns `header`, `header_2`, `header_3`, ... for each segment. |
...
| **price_supply** | `Vector` or `Dict` | Vector of numbers, vector of vectors, or timeseries spec | [0.0] | Piecewise linear price for supply curves. **Legacy**: `[40.0, 60.0, 80.0]` — one value per segment, constant over time. **Time-varying**: `[[40,41,...], [60,61,...], ...]`. **From file**: `{"timeseries": {"path": "system/price_supply.csv", "header": "bioherb_MA", "segments": 3}}` — CSV columns `header`, `header_2`, `header_3`, ... |
