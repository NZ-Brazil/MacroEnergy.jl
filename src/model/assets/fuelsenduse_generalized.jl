...
function make(asset_type::Type{GeneralFuelsEndUse}, data::AbstractDict{Symbol,Any}, system::System)
    ...
    # 우선순위: fuel_demand_edge.commodity > fuel_demand_commodity > fuel_edge.commodity (fallback)
    ...
    co2_start_node = generalfuelsenduse_transform
    @end_vertex(
        co2_end_node,
        co2_edge_data,
        CO2,
    ...
    # --- co2_captured_edge (항상 생성, capture_rate=0이면 balance에서 0으로 처리) ---
    ...
    # --- balance_data ---
    # :fuel_demand : input * conversion_rate == output
    # :emissions   : input * emission_rate   == co2 (연소 배출)
    # :co2_content : input * co2_content     == co2 (연료 내 탄소 함량)
    # :capture     : input * capture_rate    == co2_captured (capture_rate=0이면 비활성)
    ...
    return GeneralFuelsEndUse(id, generalfuelsenduse_transform, fuel_edge, fuel_demand_edge, co2_edge, co2_captured_edge)
end