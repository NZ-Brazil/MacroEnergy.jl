...
\end{aligned}
```
for each segment `s` in `segments_non_served_demand(n)` and each time `t` in `time_steps(n)` for the node `n`. The function `segments_non_served_demand` returns the segments of the non-served demand of the node `n` as defined in the input data `nodes.json`.
"""
