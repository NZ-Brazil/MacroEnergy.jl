...
function default_data(t::Type{HEFAv2}, id=missing, style="full")
    if style == "full"
...
function full_default_data(::Type{HEFAv2}, id=missing)
...
function simple_default_data(::Type{HEFAv2}, id=missing)
    return OrderedDict{Symbol,Any}(
        :id => id,
        :location => missing,
        :can_expand => true,
...
function make(asset_type::Type{HEFAv2}, data::AbstractDict{Symbol,Any}, system::System)
    id = AssetId(data[:id])
    ...
    commodity = commodity_types()[commodity_symbol]
    gasoline_start_node = hefa_transform
    @end_vertex(
        gasoline_end_node,
        gasoline_edge_data,
        commodity,
    ...
    # --- balance_data ---
    # Todas as taxas expressas em relação ao biomass_edge (edge com capacidade / capacity-bearing)
    #
    # :hvo_production      : biomass * hvo_production       == hvo_edge
    # :saf_production      : biomass * saf_production       == saf_edge
    # :gasoline_production : biomass * gasoline_production  == gasoline_edge
    # :hydrogen_consumption: biomass * hydrogen_consumption == hydrogen_edge (consumo, logo -1)
    # :elec_consumption    : biomass * electricity_consumption == elec_edge (consumo, logo -1)
    # :negative_emissions  : biomass * co2_content           == co2_edge (captura de biogênico, logo -1)
